...
def run_mimrec(input_path, output_path, mass_model_path, config_file_path):
	...
	for file in os.listdir(input_path):
		if os.path.isfile(input_path + file) and file.split('.')[-2] == 'sim' and not os.path.exists(output_path + file.split('.')[0] + '.inc1.id1.tra.gz'):
			print('Extracting events for ' + file)
			os.chdir(input_path)
			os.system('mimrec -f ' + file + ' -g ' + mass_model_path + ' -c ' + config_file_path + ' -n -x > ' + output_path + 'output_files/output_' + file.split('.')[0] + '.txt')
			os.chdir(cwd)
			os.system('mv ' + input_path + file.split('.')[0] + '.inc1.id1.extracted.tra ' + output_path + file.split('.')[0] + '.inc1.id1.extracted.tra')
...
def main():
	...
	run_mimrec(input_path, output_path, mass_model_path, config_file_path)
